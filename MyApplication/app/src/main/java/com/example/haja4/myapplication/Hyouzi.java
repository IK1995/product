...
public class Hyouzi extends Activity {
    ...
    public int number(int m, int k) {//画像を切り替えるメソッド　mはモード、iは問題番号
        int[][] Id = {{R.drawable.mario, R.drawable.charac,R.drawable.ppppp},
                {R.drawable.ppppp, R.drawable.suzukiface,R.drawable.ppppp}};//画像を収容するint配列　IDのためintらしいです
                                                            //紫？drawable.～～の「～～」が画像ファイル名です(pngとかはいらない)
        return Id[m][k];
    }

    public String Judge(int x,int y,int i){//判定用のメソッド
        String judge = "不正解...";
        //boolean judge = false;
